...
class ScrnaMetabolic(Pipeline):
    """Metabolic landscape analysis for scRNA-seq data

    An abstract from
    https://github.com/LocasaleLab/Single-Cell-Metabolic-Landscape

    If you have clustering done somewhere else, you could specify
    `config.pipeline.scrna_metabolic.clustered` to `True`
    ...
    defaults = config.pipeline.scrna_metabolic
    ...
    def build(self) -> None:
        ...
        class MetabolicInputs(Proc):
            ...
            Input:
                metafile: A metafile indicating the metadata or the rds file
                    with seruat object if
                    `config.pipeline.scrna_metabolic.clustered` is `True`.
                    For a meta file, Two columns are required: `Sample` and
                    `RNADir` `Sample` should be the first column with unique
                    identifiers for the samples and `RNADir` indicates where
                    the expression matrices are.
                    Currently only 10X data is supported
                gmtfile: The GMT file with the metabolic pathways
                config: The configuration file, string in TOML format or
                    a python dictionary as config for the analysis
                    ...
                    alias - The alias of the subset working as a prefix to
                        subset names
        ...
        self.starts.append(MetabolicInputs)

        class MetabolicSeuratPreparing(SeuratPreparing):
            if not self.options["clustered"]:
                requires = MetabolicInputs

        self.procs.MetabolicSeuratPreparing = MetabolicSeuratPreparing

        class MetabolicSeuratClustering(SeuratClustering):
            requires = MetabolicSeuratPreparing

        self.procs.MetabolicSeuratClustering = MetabolicSeuratClustering
        ...
        self.procs.MetabolicCellGroups = MetabolicCellGroups
        ...
        self.procs.MetabolicCellSubsets = MetabolicCellSubsets
        ...
        self.procs.MetabolicExprImputation = MetabolicExprImputation
        ...
        self.procs.MetabolicPrepareSCE = MetabolicPrepareSCE
        ...
        self.procs.MetabolicExprNormalization = MetabolicExprNormalization
        ...
        class MetabolicPathwayActivity(Proc):
            ...
            lang = config.lang.rscript
            script = (
                "file://../scripts/scrna_metabolic/MetabolicPathwayActivity.R"
            )
            plugin_opts = {
                "report": (
                    "file://"
        ...
        self.procs.MetabolicPathwayActivity = MetabolicPathwayActivity
        self.ends.append(MetabolicPathwayActivity)
        ...
        self.procs.MetabolicPathwayHeterogeneity = MetabolicPathwayHeterogeneity
        self.ends.append(MetabolicPathwayHeterogeneity)
        ...
        self.procs.MetabolicFeatures = MetabolicFeatures
        self.ends.append(MetabolicFeatures)
        ...
        class MetabolicFeaturesIntraSubsets(Proc):
            ...
            output = (
                "outdir:dir:{{in.configfile | stem}}.intras-pathwayfeatures"
            )
            lang = config.lang.rscript
            order = 4
            envs = {
        ...
        self.procs.MetabolicFeaturesIntraSubsets = MetabolicFeaturesIntraSubsets
        if self.options["intra-subset"]:
            self.ends.append(MetabolicFeaturesIntraSubsets)
