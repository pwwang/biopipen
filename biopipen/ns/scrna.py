...
class CellTypeAnnotation(Proc):
    ...
    - Cell-level tools: annotate the cells, each cell being assigned one cell type.
      ...
      `cheetah`, `scclassify`, `scpred`, `mapquery`, `maca`, and `scmapnet`.
    ...
    - `hitype` can run at either level: cell-level when `envs.ident` is not set,
      ...
      as a whole and assigns its cell type directly, not by majority vote).
    ...
    - Model/reference-based tools: take a trained model or a reference object,
      ...
      `envs.ident` (or `envs.cases.X.ident`), not from the tool itself.
    ...
    Envs:
        tool (choice): The tool to use for cell type annotation.
            ...
            - hitype (cell-level): Use `hitype` to annotate cell types.
            ...
            - garnett (cell-level): Use `Garnett` to annotate cell types with a
                pre-trained classifier (trained from marker genes and expression data).
        ...
        scagenttype (ns): The arguments for the scAgentType annotation agent
            ...
            - <more>: Other arguments for [`AnnotationAgent()`](https://github.com/sathyasjali/scAgentType),
                ...
                `enable_cellxgene`.
        ucell (ns): The arguments for [`UCell::AddModuleScore_UCell()`](https://github.com/carmonalab/UCell)
            ...
            score (see the marker-based note above).
            - db (type=str): The path to the marker table (required).
                Must be a universal marker table (see the note above).
            - assay (type=str): The assay to score. If not specified,
                the default assay will be used.
            - maxRank (type=int): The number of top-ranked genes used to score
                a signature (default: 1000).
            - w_neg (type=float): The weight of the negative markers
                (default: 1).
            - name (type=str): The suffix of the score columns added to the
                metadata (default: `_UCell`).
            - tissue: Filter the markers by the `tissue` column of the universal
                marker table (see the note above).
            - cancer: Filter the markers by the `cancer` column of the universal
                marker table (see the note above).
            - species: Filter the markers by the `species` column of the universal
                marker table (see the note above).
        aucell (ns): The arguments for [`AUCell::AUCell_calcAUC()`](https://github.com/aertslab/AUCell)
            if `tool` is `aucell`. The negative-direction markers of the marker
            table are dropped (a ranked list has no direction).
            - db (type=str): The path to the marker table (required).
                Must be a universal marker table (see the note above).
            - assay (type=str): The assay to score. If not specified,
                the default assay will be used.
            - aucMaxRank (type=int): The number of top-ranked genes to
                calculate the AUC on (default: 5% of the genes).
            - normAUC (flag): Whether to normalize the AUC to the maximum
                possible AUC (default: TRUE).
            - tissue (type=str): Filter the marker table by tissue, e.g.
                `Immune system`.
            - cancer (type=str): Filter the marker table by cancer, e.g.
                `Breast cancer`.
            - species (type=str): Filter the marker table by species, e.g.
                `Human`.
        gsva (ns): The arguments for [`GSVA::gsva()`](https://bioconductor.org/packages/release/bioc/html/GSVA.html)
            if `tool` is `gsva`. The negative-direction markers of the marker
            table are dropped (a gene set has no direction).
            - db (type=str): The path to the marker table (required).
                Must be a universal marker table (see the note above).
            - assay (type=str): The assay to score. If not specified,
                the default assay will be used.
            - kcdf (choice): The kernel to use for the enrichment scores.
                `Gaussian` (default), `Poisson`, or `none`.
            - minSize (type=int): The minimum number of genes in a gene set
                (default: 1).
            - maxSize (type=int): The maximum number of genes in a gene set
                (default: `Inf`, i.e. no limit).
            - tissue (type=str): Filter the marker table by tissue, e.g.
                `Immune system`.
            - cancer (type=str): Filter the marker table by cancer, e.g.
                `Breast cancer`.
            - species (type=str): Filter the marker table by species, e.g.
                `Human`.
        singscore (ns): The arguments for [`singscore::simpleScore()`](https://bioconductor.org/packages/release/bioc/html/singscore.html)
            if `tool` is `singscore`. Unlike the other scorers, the direction of
            the markers is used as-is (`upSet`/`downSet`).
            - db (type=str): The path to the marker table (required).
                Must be a universal marker table (see the note above).
            - assay (type=str): The assay to score. If not specified,
                the default assay will be used.
            - subSamples (type=int): The number of random subsets of the ranked
                genes to score. Cells left out are assigned `NA`.
            - centerScore (flag): Whether to center the scores to a [0, 1]
                range.
            - tissue (type=str): Filter the marker table by tissue, e.g.
                `Immune system`.
            - cancer (type=str): Filter the marker table by cancer, e.g.
            ...
            - species (type=str): Filter the marker table by species, e.g.
                `Human`.
        scmap (ns): The arguments for `scmap::scmapCluster()` (`scmap::scmapCell()`
            ...
            with a `logcounts` assay.
            - db (type=str): The path to the reference (required).
            - assay (type=str): The assay of the reference and of the object
                (default: `RNA`).
            - cluster_col (type=str): The column of the reference's `colData`
                holding the cell types (default: `cell_type1`).
            - features (type=auto): The features used for the scmap index.
                Defaults to all the features of the reference.
            - threshold (type=float): The similarity threshold below which a
                cell is labeled `unassigned` (default: 0.5).
            - use_cell_index (flag): Use the scmap-cell (k-means) index instead
                of the scmap-cluster index (default: FALSE).
        cheetah (ns): The arguments for [`CHETAH::CHETAHclassifier()`](https://bioconductor.org/packages/release/bioc/html/CHETAH.html)
            if `tool` is `cheetah`. The reference is a `SingleCellExperiment`
            (or a Seurat object, converted on the fly) holding counts.
            - db (type=str): The path to the reference (required).
            - assay (type=str): The assay of the reference and of the object
                (default: `RNA`).
            - input_c (type=str): The name of the assay of the input to use.
            - thresh (type=float): The confidence threshold below which a cell
                is labeled `Unassigned` (default: 0.1).
            - n_genes (type=int): The number of genes used for the classification.
            - pc_thresh (type=float): The threshold for the principal components.
            - only_pos (flag): Whether to use only the positively correlated
                genes. Defaults to the CHETAH default.
            - ref_ct (type=str): The reference's cell-type column name
                (auto-detected when not given).
            - label (type=str): The label vector/column used when the reference
                carries several.
        scclassify (ns): The arguments for `scClassify::scClassify()` /
            `scClassify::predict_scClassify()` if `tool` is `scclassify`.
            ...
            - db (type=str): The path to the reference (required).
            - assay (type=str): The assay to classify (default: `RNA`).
            - algorithm (choice): The algorithm to use. `WKNN` (default), `KNN`,
                or `DWKNN` (the `lr` of the scClassify docs is not supported by
                the installed scClassify).
            - topN (type=int): The number of features used by the model
                (default: 50).
            - prob_threshold (type=float): The probability threshold below which
                a cell is labeled `unassigned`.
            - parallel (flag): Whether to run scClassify in parallel.
        scpred (ns): The arguments for [`scPred::scPredict()`](https://github.com/powellgenomicslab/scPred)
            if `tool` is `scpred`. The reference is a labelled Seurat object, and
            the feature space is extracted and the model trained on it at runtime.
            - db (type=str): The path to the reference (required).
            - assay (type=str): The assay to predict.
            - pvar (type=str): The metadata column of the reference holding the
                cell types (default: `cell_type`).
            - model (type=str): The model to train (default: `svmRadial`).
            - reduction (type=str): The reduction used for the feature space
                (default: `pca`).
            - threshold (type=float): The prediction-probability threshold
                passed to `scPredict()` (default: 0.55).
        azimuth (ns): The arguments for [`Azimuth::RunAzimuth()`](https://github.com/satijalab/azimuth)
            ...
            its cells.
            - ref (type=str): The reference name (looked up through `SeuratData`,
                e.g. `pbmcref`, downloaded when not installed) or the path to a
                directory holding `ref.Rds` + `idx.annoy` (required).
            - db (type=str): An alias of `ref`.
            - assay (type=str): The assay to transfer.
            - anno_col_in (type=str): The `predicted.*` column of the Azimuth
                result to use as the annotation. Defaults to
                `predicted.celltype.l1` when present, otherwise the first
                `predicted.*` column.
            - dims: Ignored with a warning: the installed Azimuth reads the
        ...
        maca (ns): The arguments for [`MACA`](https://github.com/ImXman/MACA)
            ...
            `biopipen/scripts/scrna/maca-wrapper.py`.
            - db (type=str): The path to the marker table (required).
                Must be a universal marker table (see the note above).
            - python (type=str): The python path with MACA installed
                (default: the same python as the pipeline).
            - n_pcs (type=int): The number of principal components.
            - res (type=auto): The Louvain resolutions, e.g. `[1, 2, 3]`
                (default: MACA's own `[1, 2, 3]`).
            - n_neis (type=auto): The numbers of neighbors, e.g. `[5, 10]`
                (default: MACA's own `[5, 10]`).
            - freq (type=float): The frequency threshold of the cluster mapping
                (default: 0.5).
            - use_weight (flag): Whether to weight the markers by their order in
                the marker table (default: FALSE).
        scmapnet (ns): The arguments for [`scMapNet`](https://github.com/Yuz7/scMapNet)
            ...
            `biopipen/scripts/scrna/scmapnet-wrapper.py`.
            The pre-trained weights are a manual download and are licensed
            CC BY-NC 4.0 (non-commercial).
            - db (type=str): The path to the marker table (required).
                Must be a universal marker table (see the note above).
            - python (type=str): The python path with the scMapNet dependencies
                (`torch`, `timm`, `torchvision`) installed
                (default: the same python as the pipeline).
            - scmapnet_dir (type=str): The path to the cloned scMapNet repo
                (required), i.e. the directory holding `main_finetune.py` and
                `generate_image_script.sh`.
            - weights (type=str): The path to the checkpoint used for the
                ...
                the cell types are indexed in their sorted order.
            - organ (type=str): The organ of the cells. scMapNet builds the
                treemap images from an organ → cell type → gene hierarchy, so
                this is the `organ` of every marker in the table.
        mllmcelltype (ns): The arguments for
            `mLLMCelltype::annotate_cell_types()` if `tool` is `mllmcelltype`.
            One call is made per run, with the top markers of every cluster.
            - tissue (type=str): The tissue of the cells (required),
                e.g. `human PBMC`.
            - model (type=str): The LLM to use (default: `gpt-5.5`).
                The model decides the provider, and so which API key is needed.
            - api_key (type=str): The API key of the provider. When not set, the
                key is read from the environment variable of the provider
                (`OPENAI_API_KEY` or `ANTHROPIC_API_KEY`); without one, only the
                prompt is built and an error is raised instead of annotating.
        ...
        lict (ns): The arguments for `LICT::LLMCellType()` if `tool` is `lict`.
            ...
            - species (type=str): The species of the cells (default: `Human`).
            - tissue (type=str): The tissue of the cells.
            - topgenenumber (type=int): The number of top markers per cluster
                put into the prompt (default: 30).
            - validate (flag): Whether to validate the markers
                (default: TRUE).
            - percent (type=float): The percentage of the markers to use
                (default: 0.5).
            - keys (type=json): The API keys of the providers, as a map from the
                environment variable name to its value, e.g.
                `{ "openai_api_key": "<key>" }`. They are set in the R session
                (and inherited by the child processes) before the call.
            - provider (type=str): The provider whose answer to use. Defaults to
                the first provider that answered.
        mapquery (ns): The arguments for [`Seurat::MapQuery()`](https://satijalab.org/seurat/reference/mapquery)
            ...
            also gets the majority call of its cells when `envs.ident` is set.
            - db (type=str): The reference Seurat object file (required).
            - use (type=str): The metadata column of the reference holding the
                cell types to transfer (required).
            - ident_name (type=str): The metadata column of the query that receives
                the transferred labels (kept distinct from `ident` so the clustering
                column is not overwritten) (default: `predicted.id`).
            - refnorm (choice): The normalization method the reference used; the
                same method is used for the query (default: `auto`).
                - LogNormalize: Normalize the query with `NormalizeData()`.
                - SCT: Normalize the query with `SCTransform()`.
                - SCTransform: An alias of `SCT`.
                - auto: Automatically detect the normalization method.
            - skip_if_normalized (flag): Whether to skip the normalization if the
                query is already normalized (default: TRUE).
            - ncores (type=int): Number of cores to use for the mapping.
            - map_query (ns): The arguments for `Seurat::MapQuery()`.
            - find_transfer_anchors (ns): The arguments for `Seurat::FindTransferAnchors()`.
            - sctransform (ns): The arguments for `Seurat::SCTransform()`.
            - normalize_data (ns): The arguments for `Seurat::NormalizeData()`.
...
class GarnettClassifierTrainer(Proc):
    ...
    Envs:
        ...
        cds_gene_id_type (choice): The gene ID type of the expression data
            (default: `custom`). Supported types include `SYMBOL`, `ENSEMBL`,
            `ENTREZID`.
...
class CellCellCommunicationPlots(Proc):
    ...
    Envs:
        ...
        ligand_expr: The name of the column containing the mean (or otherwise summarized) expression of the ligand.
            ...
            If not specified, `ligand_means` will be tried first, then `ligand_trimean`.
        receptor_expr: The name of the column containing the mean (or otherwise summarized) expression of the receptor.
            Only used when plot_type = "linkedheatmap".
            If not specified, `receptor_means` will be tried first, then `receptor_trimean`.
...
class Slingshot(Proc):
    ...
    Envs:
        ...
        dims (type=auto): The dimensions to use for the analysis.
            ...
            Or a single number greater than 1, which will be expanded to `1:number`.
