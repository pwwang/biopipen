...
#                                                                ])
#             else:
#                 indata[inkey + ':' + intype] = Channel.create(
#                   opts.i.get(inkey))
...
ROOT_MODULE = Path(__file__).parent.parent.name

class CommandABC(ABC):
    """Abstract class for commands and modules that are used for cli: biopipen
    """
    @abstractclassmethod
    def collections(cls):
        """Get a collections this this type of commands"""

    @abstractclassmethod
    def use_cache(cls, cache_file: Path):
        """Check if we can use the cached file to load params"""
    ...
    @abstractclassmethod
    def is_a(cls, name: str):
        """Check if a command is my type"""
...
def file_newer(file1: PathType,
               files2: Union[PathType, Iterator[PathType]]) -> bool:
    """Tell if file1 is newer"""
    if isinstance(files2, (str, Path)):
        files2 = [files2]
...
def import_from_path(path: Path):
    """Modkit doesn't work with importlib.util.spec_from_file_location,
    we have to use __import__

    This function only supports paths under ROOT_MODULE
    """
    ...
    module = __import__(ROOT_MODULE + dirname, fromlist=[path.stem])
