...
from vireoSNP.plot import heat_matrix, anno_heat  # type: ignore
...
ad_data = mmread(ad_file).tocsc()  # type: ignore
dp_data = mmread(dp_file).tocsc()  # type: ignore

if ad_data.shape[0] < 2:  # type: ignore
    logger.warning(
        f"AD matrix has only {ad_data.shape[0]} variants, "  # type: ignore
        "VireoSNP requires at least 2 variants to run. "
    )
    logger.warning("Generating empty output files and exiting...")
    Path(f"{outdir}/cell_clone_assignment.tsv").write_text(
        "cell_id\tassigned_clone\tassignment_prob\n"
    )
    ...
    plt.text(
        0.5,
        0.5,
        f"Only {ad_data.shape[0]} variants found,\n"  # type: ignore
        "VireoSNP requires at least 2 variants to run.",
...
if cell_number_info_file.exists():
    ...
    for _, row in cell_info_df.iterrows():
        ...
        col_offset += n_cells
    logger.info(
        f"Loaded sample information for {len(cell_info_df)} samples, {col_offset} total cells"
    )
else:
...
def fit_model_for_k(k):
    ...
    _model = BinomMixtureVB(
        n_var=ad_data.shape[0],  # type: ignore
        n_cell=ad_data.shape[1],  # type: ignore
        n_donor=k,
    )
    _model.fit(
        ad_data,
...
if isinstance(n_clones, (tuple, list)):
    ...
    logger.info(
        f"Testing n_clones from {n_clones[0]} to {n_clones[1]} using {ncores} cores..."
    )
...
plt.subplot(1, 2, 1)
im = heat_matrix(
    _model.ID_prob, cmap="Blues", alpha=0.8, display_value=False, row_sort=True
)
plt.colorbar(im, fraction=0.046, pad=0.04)
plt.title("Assignment probability")
plt.xlabel("Clone")
plt.ylabel("%d cells" % (_model.n_cell))
plt.xticks(range(_model.n_donor))  # type: ignore


plt.subplot(1, 2, 2)
im = heat_matrix(
    _model.beta_mu,
    cmap=segpink,  # type: ignore
    alpha=0.8,  # type: ignore
    display_value=False,
    row_sort=True,
)
plt.colorbar(im, fraction=0.046, pad=0.04)
...
if mtSNP_ids is None:
    mtSNP_ids = ["variant%d" % x for x in range(ad_data.shape[0])]  # type: ignore
    ...
    **kwargs,
    ...
    if col_anno is not None:
        ...
        if col_anno2 is not None:
            ...
            col_colors2 = np.array(
                [get_color_for_index(num + len(col_order_ids)) for num in col_num2]
            )[idx_col]
    ...
    if row_anno is not None:
        for i in range(len(row_order_ids)):
            g.ax_row_dendrogram.bar(
                0, 0, color=get_color_for_index(i), label=row_order_ids[i], linewidth=0
            )
    ...
    if col_anno is not None:
        ...
        for i in range(len(col_order_ids)):
            handle = g.ax_col_dendrogram.bar(
                0, 0, color=get_color_for_index(i), label=col_order_ids[i], linewidth=0
            )
        ...
        if col_anno2 is not None:
            sample_handles = []
            for i in range(len(col_order_ids2)):  # type: ignore
                handle = g.ax_col_dendrogram.bar(
                    0,
                    0,
                    color=get_color_for_index(i + len(col_order_ids)),
                    label=col_order_ids2[i],  # type: ignore
                    linewidth=0,
                )  # type: ignore
            ...
            g.ax_col_dendrogram.legend(
                ...
                loc="lower center",
                ncol=min(len(col_order_ids2), 6),  # type: ignore
                title="Samples",
                frameon=True,
            )
        else:
    ...
    g.cax.set_position([1.01, 0.2, 0.03, 0.45])  # type: ignore
...
n_cells = ad_data.shape[1]  # type: ignore
n_vars = len(var_idx)
...
logger.info(
    f"Heatmap dimensions: {fig_width:.1f}x{fig_height:.1f} inches ({n_cells} cells x {n_vars} variants)"
)
