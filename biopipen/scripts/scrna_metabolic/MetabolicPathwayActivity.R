...
norm_tpm <- assay(sce, "tpm")

pathways <- gmtPathways(gmtfile)
pathway_names <- names(pathways)
all_cell_types <- as.vector(sce[[config$grouping_name]])
cell_types <- unique(all_cell_types)
metabolics <- unique(as.vector(unname(unlist(pathways))))

gene_pathway_number <- num_of_pathways(
    gmtfile,
    rownames(sce)[rowData(sce)$metabolic]
)
...
heatmapfile = file.path(outdir, "KEGGpathway_activity_heatmap.png")
...
violinfile <- file.path(outdir, "pathway_activity_violinplot.png")
plotViolin(
    scRNA_df,
    args = list(
