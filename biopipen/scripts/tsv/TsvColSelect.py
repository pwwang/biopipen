...
from biopipen.utils import always_list
from biopipen.utils.tsvio import TsvReader, TsvWriter

{% python is_lambda = lambda x: isinstance(x, str) and x.startswith('lambda ') %}
infile = {{in.infile | quote}}
colfile = {{in.colfile | repr}}
outfile = {{out.outfile | quote}}
inopts = {{args.inopts | repr}}
cols = {{args.cols | is_lambda ? !repr }}
keep = {{args.keep | repr}}

from_file = False
if colfile and path.isfile(colfile):
