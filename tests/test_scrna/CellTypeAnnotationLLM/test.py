...
BASE_URL = os.environ.get("LLMCELLTYPE_BASE_URL")
API_KEY = os.environ.get("OPENAI_API_KEY")
if not API_KEY:
    raise SystemExit(
        "OPENAI_API_KEY is not set. This test makes real OpenAI API calls "
        "via the LLMCelltype R package and cannot run without it."
    )

MODEL = os.environ.get("LLMCELLTYPE_MODEL", "gpt-4o-mini")
...
class PrepData(Proc):
    ...
    script = """
        ...
        # llmcelltype() API call is made and the run stays quick.
...
class CellTypeAnnotationLLM(CellTypeAnnotation_):
    requires = PrepData
    envs = {
        "tool": "llmcelltype",
        "llmcelltype": {
...
def testing(pipen):
    # llmcelltype (cluster-level): CellType column, no cell2celltype.tsv.
    # Cell-type names come from the model, so only assert the plumbing.
    proc = get_proc(pipen, "CellTypeAnnotationLLM")
    outfile = proc.workdir.joinpath("0", "output", "pbmc3k.annotated.RDS")
    cols, idents, ncells = get_rds_info(pipen, "CellTypeAnnotationLLM")
