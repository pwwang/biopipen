...
class TestCommon (helpers.TestCase):
	...
	testdir, indir, outdir = helpers.testdirs('TestCommon')

	def dataProvider_testSort(self):
		infile = path.join(self.indir, 'sort.txt')
		yield 't1', infile, {}, {'k': 2}, True, False, path.join(self.outdir, 'sort.txt')
		yield 't2', infile, {'skip': 3}, {'k': 2}, True, False, path.join(self.outdir, 'sort-skip.txt')
		yield 't3', infile, {}, {'k': 2}, False, False, path.join(self.outdir, 'sort-case.txt')
		yield 't4', infile, {}, {'k': 1}, True, True, path.join(self.outdir, 'sort-unique.txt')
		yield 't5', infile, {'delimit': '|'}, {'k': 2}, True, True, path.join(self.outdir, 'sort-delimit.txt')
	...
	def dataProvider_testFiles2Dir(self):
		files1 = glob(path.join(self.indir, 'files2dir*.txt'))
		yield 't1', files1, [path.basename(f) for f in files1]
		files2 = glob(path.join(self.indir, 'files2dirSameBn*', 'samefile.txt'))
	...
	def dataProvider_testStr2File(self):
		infile = path.join(self.indir, 'sort.txt')
		yield 't1', infile
	...
	def dataProvider_testAddHeader(self):
		infile1 = path.join(self.indir, 'sort.txt')
		infile2 = path.join(self.indir, 'sort.txt')
		outfile = path.join(self.outdir, 'addheader.txt')
	...
	def dataProvider_testMergeFiles(self):
		files   = [path.join(self.indir, 'sort.txt')] * 2
		inopts  = {'skip': [0, 2], 'comment': ''}
		outfile = path.join(self.outdir, 'mergefiles.txt')
	...
	def dataProvider_testFile2Proc(self):
		infile = path.join(self.indir, 'sort.txt')
		yield 't1', infile
	...
	def dataProvider_testHead(self):
		infile1 = path.join(self.indir, 'sort.txt')
		outfile1 = path.join(self.outdir, 'head.txt')
	...
	def dataProvider_testTail(self):
		infile1 = path.join(self.indir, 'sort.txt')
		outfile1 = path.join(self.outdir, 'tail.txt')
		yield 't1', infile1, outfile1
	...
	def dataProvider_testPrepend(self):
		infile = path.join(self.indir, 'sort.txt')
		outfile = path.join(self.outdir, 'prepend.txt')
	...
	def dataProvider_testAppend(self):
		infile = path.join(self.indir, 'sort.txt')
		outfile = path.join(self.outdir, 'append.txt')
	...
	def dataProvider_testSplitRows(self):
		infile  = path.join(self.indir, 'sort.txt')
		outfile = path.join(self.outdir, 'splitrows.rows')
