...
if [[ "$LOCAL_ONLY-$FORCE" == "true-false" ]]; then
    ...
    CMD="conda run --no-capture-output -n $ENVNAME uv run $PROC_TEST_DIR/test.py $ARGS"
    echo "  Running: $CMD"

    if [[ $VERBOSE == "true" ]]; then
        conda run --no-capture-output -n $ENVNAME uv run $PROC_TEST_DIR/test.py $ARGS
    else
        conda run --no-capture-output -n $ENVNAME uv run $PROC_TEST_DIR/test.py $ARGS > /dev/null 2>&1
    fi
