...
class TestUtilsGene(helpers.TestCase):
	...
	def dataProvider_testGenenorm(self, testdir, indir):
		...
		yield infile, None, 'skip', 'symbol, alias', 'symbol', 'hg19', {'ftype': ['Gene', 'Index']}, {}, None, testdir, {
	...
	def dataProvider_testGenenormCached(self, testdir, indir):
		...
		genenorm(infile2, None, 'skip', 'symbol, alias', 'alias', 'hg19', {'ftype': ['Gene', 'Index']}, {}, None, cachedir)
		
		yield infile, None, 'skip', 'symbol, alias', 'symbol', 'hg19', {'ftype': ['Gene', 'Index']}, {}, None, cachedir, {
	...
	def dataProvider_testGenenormOutfile(self, testdir, indir, outdir):
		...
		makedirs(cachedir)
		# cache part of the query
		genenorm(infile, None, 'skip', 'symbol, alias', 'alias', 'hg19', {'ftype': ['Gene', 'Index']}, {}, None, cachedir)
		
		outfile = path.join(testdir, 'genenorm.out')
		exptfile = path.join(outdir, 'genenorm.out')
		
		yield infile, outfile, 'ignore', 'symbol, alias', 'symbol', 'hg19', {'ftype': ['Gene', 'Index']}, {'query': True}, None, cachedir, exptfile
