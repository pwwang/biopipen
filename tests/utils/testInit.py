...
class TestInit(helpers.TestCase):
	...
	def testRuncmd(self, cmd, ret, quit, exception = None):
		if exception:
			with self.assertLogs('bioprocs', 'DEBUG') as cm:
				self.assertRaises(exception, runcmd, cmd, quit)
			self.assertInAny('Running command at PID:', cm.output)
			self.assertInAny(cmd, cm.output)
			self.assertInAny('Return code: ', cm.output)
		else:
			with self.assertLogs('bioprocs', 'DEBUG') as cm:
				r = runcmd(cmd, quit)
