...
class TestUtilsSnp(helpers.TestCase):
	...
	testdir, indir, outdir = helpers.testdirs('TestUtilsSnp')

	def dataProvider_testSnpinfo(self):
		infile = path.join(self.indir, 'snps.txt')
		yield infile, None, 'skip', 'hg19', '150', {'cnames': ['Snp', 'Index']}, {}, None, self.testdir, {
	...
	def dataProvider_testSnpinfoCached(self):
		infile = path.join(self.indir, 'snps.txt')
	...
	def dataProvider_testSnpinfoOutfile(self):
		infile = path.join(self.indir, 'snps.txt')
		cachedir = path.join(self.testdir, 'cached')
		...
		outfile = path.join(self.testdir, 'snpinfo.txt')
		exptfile = path.join(self.outdir, 'snpinfo.txt')
	...
	def testSnpinfoOutfile(self, infile, outfile, notfound, genome, dbsnpver, inopts, outopts, snpcol, cachedir, exptfile):
		...
		self.assertFileCountEqual(outfile, exptfile)
