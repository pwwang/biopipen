...
class TestSambam (unittest.TestCase):
	...
	def test0_getRef (self):
		...
		pDownloadGet.input    = {pDownloadGet.input: ['https://raw.githubusercontent.com/BenLangmead/bowtie2/master/example/reference/lambda_virus.fa']}
		pChangeChrom          = proc (desc = 'Change the chrom to regular name.')
		pChangeChrom.depends  = pDownloadGet
		pChangeChrom.input    = 'infile:file'
		pChangeChrom.output   = 'outfile:file:{{infile | bn}}'
		pChangeChrom.lang     = 'python'
		pChangeChrom.script   = """
		# Indent: remove
		from random import choice
		open("{{outfile}}", "w").write(
			open("{{infile}}").read().replace(">gi|9626243|ref|NC_001416.1| Enterobacteria phage lambda, complete genome", ">chr1") + 
			">chr2\\n" +
	...
	def test1_pFastqPE2Sam (self):
		...
		pFastqPESimWgsim.input       = {pFastqPESim.input: [1]}
		pFastqPESimDwgsim.input      = {pFastqPESim.input: [1]}
	...
	def test2_pFastqPE2Sam (self):
		...
		pFastqPE2Sam.input            = {pFastqPE2Sam.input: self.data.fqs}
	...
	def test3_Sam2BamSambamba (self):
		...
		pyppl().starts(pSam2BamSbb1, pSam2BamSbb2, pSam2BamSbb3).run()
	...
	def test3_Sam2BamSamtools (self):
		...
		pyppl().starts(pSam2BamSmt1, pSam2BamSmt2, pSam2BamSmt3).run()
	...
	def test3_Sam2BamPicard (self):
		...
		pyppl().starts(pSam2BamPcd1, pSam2BamPcd2, pSam2BamPcd3).run()

	
	def test3_1Sam2BamBiobambam (self):
		pSam2Bam.forks= 2
		pSam2Bam.input= {pSam2Bam.input: self.data.sams}
		...
		pyppl().starts(pSam2BamBbb1, pSam2BamBbb2, pSam2BamBbb3).run()
		
	def test4_BamMarkdup (self):
		pBamMarkdup.args.nthread = 2
		pBamMarkdup.forks = 4
		pBamMarkdup.input = {pBamMarkdup.input: self.data.bams}
		...
		pyppl().starts(pBamMarkdup1,pBamMarkdup2,pBamMarkdup4,pBamMarkdup5).run()
				
	def test2_FastqSE2Sam (self):
		pFastqSE2Sam.args.nthread = 2
		pFastqSE2Sam.forks = 8
		pFastqSE2Sam.input = {pFastqSE2Sam.input: channel(self.data.fqs).fold(1)}
		...
		pyppl().starts(pFastqSE2Sam1, pFastqSE2Sam2, pFastqSE2Sam3).run()
	
	
	def test4_BamRecal (self):
		pBamRecalTest = pBamRecal.copy()
		pBamRecalTest.input = {pBamRecal.input: self.data.bams}
		...
		pyppl().starts(pBamRecalTest).run()
		
	def test4_BamReadGroup (self):
		pBamReadGroup1 = pBamReadGroup.copy()
		pBamReadGroup2 = pBamReadGroup.copy()
		pBamReadGroup1.input = {pBamReadGroup.input: self.data.bams}
		pBamReadGroup2.input = {pBamReadGroup.input: self.data.bams}
		pBamReadGroup2.args.tool = 'bamutil'
		
		pyppl().starts(pBamReadGroup1, pBamReadGroup2).run()
	
	def test4_BamReorder (self):
		
		pBamReorder.args.ref = params.ref
		pBamReorder.input = {pBamReorder.input: self.data.bams}
		
		pyppl().starts(pBamReorder).run()
	
	def test4_Mergebams (self):
		pFiles2List.input = {pFiles2List.input: [self.data.bams.toList()]}
	...
	def test4_pBam2Gmut (self):
		...
		pBam2Gmut.forks    = 10
		pBam2Gmut.input    = {pBam2Gmut.input: self.data.bams}
	...
	def test4_pBamPair2Smut (self):
		pBamPair2Smut.input        = {pBamPair2Smut.input: self.data.bams.unfold(2)}
	...
	def test5_pBam2Cnv (self):
		pBam2Cnv.input = {pBam2Cnv.input: self.data.bams}
	...
	def test6_pBamStats (self):
		pBamStats.input = {pBamStats.input: self.data.bams}
		pBamStats.forks = 10
