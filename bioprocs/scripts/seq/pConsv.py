...
bedfile = path.join({{job.outdir | quote}}, "{{in.bedfile | bn}}.sorted")
cmd = 'grep -v "^#" {{in.bedfile | quote}} | cut -f1,2,3,4,5,6  | sort -k1,1 -k2,2n > "%s"' % bedfile
runcmd(cmd)
...
with open({{in.bedfile | quote}}) as f, open({{out.outfile | quote}}, 'w') as fout:
	for line in f:
		line  = line.strip()
		if not line: continue
		if line.startswith('#'):
