{{ 'plot.r', '__init__.r', 'sampleinfo.r' | rimport }} # nolint
...
if (!is.null(metadata)) {
    ...
    ret$allgene = cbind(metaall, ret$allgene)
}
write.xls(ret$allgene, allfile)

# get and save degs
if (cutoff$by == 'q') {
    degene = ret$allgene[ret$allgene$FDR < cutoff$value,,drop = FALSE]
} else {
    degene = ret$allgene[ret$allgene$PValue < cutoff$value,,drop = FALSE]
}
...
if (!is.logical(ggs$maplot) || ggs$maplot!=FALSE) {
    ...
    M = ret$allgene$logFC
    threshold = ret$allgene$PValue < cutoff$value
    plot.maplot(
        data.frame(A, M),
        maplot,
        threshold,
