...
datacols = {{args.datacols | R}}
inform  = {{args.inform | ?!:'NULL' }}
outform = {{args.outform | ?!:'NULL' }}

data    = read.table.inopts (infile, inopts)
annodata = NULL
if (!is.null(datacols) && length(datacols) > 0) {
    if (is.character(datacols)) {
        datacols = expand.numbers(datacols)
    }
    if (is.numeric(datacols)) {
        annodata = data[, setdiff(1:ncol(data), datacols), drop = FALSE]
    } else {
        annodata = data[, setdiff(colnames(data), datacols), drop = FALSE]
    }
    data = data[, datacols, drop = FALSE]
}
...
if (!is.null(annodata)) {
    expr = cbind(annodata[rownames(expr), , drop = FALSE], expr)
}

write.table(expr, outfile, quote=FALSE, row.names=TRUE, col.names=TRUE, sep="\t")
