...
GSEA.GeneRanking <- function(A,
    ...
    for (r in 1:nperm) {
        class1.subset <- sample(class1.index,
                                size = ceiling(class1.size*fraction),
                                replace = replace)
        class2.subset <- sample(class2.index,
                                size = ceiling(class2.size*fraction),
                                replace = replace)
        ...
        if (permutation.type == 0) { # random (unbalanced) permutation
            ...
            class1.label1.subset <- sample(
                class1.subset,
                size = ceiling(class1.subset.size*fraction.class1)
            )
            class2.label1.subset <- sample(
                class2.subset,
                size = floor(class2.subset.size*fraction.class1)
            )
            reshuffled.class.labels1[, r] <- rep(0, Ns)
            reshuffled.class.labels2[, r] <- rep(0, Ns)
            class.labels1[, r] <- rep(0, Ns)
...
GSEA.EnrichmentScore <- function(gene.list,
                                 gene.set,
    ...
    # weight: 0 (unweighted = Kolmogorov-Smirnov), 1 (weighted), and
    # 2 (over-weighted). When the score type is 1 or 2 it is
    # necessary to input the correlation vector with the values in
    # the same order as in the gene list.
    #
    # Inputs:
    #   gene.list: The ordered gene list (e.g. integers indicating the
    #              original position in the input dataset)
    #   gene.set: A gene set (e.g. integers indicating the location of
    #             those genes in the input dataset)
    #   weighted.score.type: Type of score: weight:
    #                        0 (unweighted = Kolmogorov-Smirnov),
    #                        1 (weighted), and
    #                        2 (over-weighted)
    #  correl.vector: A vector with the coorelations
    #                 (e.g. signal to noise scores) corresponding to
...
OLD.GSEA.EnrichmentScore <- function(gene.list, gene.set) {
    #
    # Computes the original GSEA score from Mootha et al 2003 of
    # gene.set in gene.list
    #
    # Inputs:
    #   gene.list: The ordered gene list (e.g. integers indicating the
    #              original position in the input dataset)
...
GSEA.EnrichmentScore2 <- function(gene.list,
                                  gene.set,
                                  weighted.score.type = 1,
                                  correl.vector = NULL) {
    #
    # Computes the weighted GSEA score of gene.set in gene.list.
    # It is the same calculation as in
    # GSEA.EnrichmentScore but faster (x8) without producing the RES,
    # arg.RES and tag.indicator outputs.
    # This call is intended to be used to asses the enrichment of
    # random permutations rather than the
    # observed one.
    # The weighted score type is the exponent of the correlation
    # weight: 0 (unweighted = Kolmogorov-Smirnov), 1 (weighted), and
    # 2 (over-weighted). When the score type is 1 or 2 it is
...
GSEA.HeatMapPlot <- function(V,
                             ...
                             ylab=" ") {
    #
    # Plots a heatmap "pinkogram" of a gene expression matrix including
    # phenotype vector and gene, sample and phenotype labels
    #
    # The Broad Institute
    # SOFTWARE COPYRIGHT NOTICE AGREEMENT
    # This software and its documentation are copyright 2003 by the
    ...
    rownames(V) = ifelse(is.logical(row.names), NULL, row.names)
    colnames(V) = ifelse(is.logical(col.names), NULL, col.names)
    group_col <- c()
    group_col[class.tags[1]] <- 'red3'
    group_col[class.tags[2]] <- 'green3'
    plot.heatmap2(V, params=list(
        ...
        row_names_side = "left",
        heatmap_legend_param = list(title="Mean-centered\nexpression",
                                    labels_gp =gpar(fontsize = 6),
                                    title_gp =gpar(fontsize = 8)),
        cluster_rows = FALSE,
        column_dend_height = unit(10, "mm"),
        top_annotation = HeatmapAnnotation(
            Expression = anno_boxplot(V),
            Group = class.tags[col.labels + 1],
            annotation_height = unit(c(15, 2), "mm"),
            annotation_name_gp = gpar(fontsize = 8),
            show_legend = TRUE,
            annotation_legend_param = list(
                Group = list(labels_gp =gpar(fontsize = 6),
                             title_gp =gpar(fontsize = 8))
            ),
    ...
    # # blue-pinkogram colors. The first and last are the colors to indicate
    # # the class vector (phenotype). This is the 1998-vintage, pre-gene cluster,
    # # original pinkogram color map
    ...
    #     axis(1, at=1:n.cols, labels=col.names, tick=FALSE, las = 1,
...
GSEA.Res2Frame <- function(filename = "NULL") {
    #
    # Reads a gene expression dataset in RES format and converts it into
    # an R data frame
    #
    # The Broad Institute
    # SOFTWARE COPYRIGHT NOTICE AGREEMENT
...
GSEA.Gct2Frame <- function(filename = "NULL") {
    #
    # Reads a gene expression dataset in GCT format and converts it into
    # an R data frame
    #
    # The Broad Institute
    # SOFTWARE COPYRIGHT NOTICE AGREEMENT
...
GSEA.ReadClsFile <- function(file = "NULL") {
    #
    # Reads a class vector CLS file and defines phenotype and class
    # labels vectors for the samples in a gene expression file
    # (RES or GCT format)
    #
    # The Broad Institute
    # SOFTWARE COPYRIGHT NOTICE AGREEMENT
    ...
    phen.tags <- unlist(strsplit(sub("^#\\s*", "", cls.cont[[2]]), " "))
    ...
    return(list(phen = phen, class.v = class.v, phen.tags = phen.tags))
}
...
GSEA <- function(input.ds,
    ...
    # This is a methodology for the analysis of global molecular profiles
    # called Gene Set Enrichment Analysis (GSEA). It determines
    ...
    # determines whether members of an a priori defined gene set are
    # nonrandomly distributed towards the top or bottom of the
    # list and thus may correspond to an important biological process.
    # To assess significance the program uses an empirical
    ...
    #
    # Inputs:
    #   input.ds: Input gene expression Affymetrix dataset file in
    #             RES or GCT format
    #   input.cls:  Input class vector (phenotype) file in CLS format
    #   gene.ann.file: Gene microarray annotation file
    #                  (Affymetrix Netaffyx *.csv format) (default: none)
    #   gs.file: Gene set database in GMT format
    #   output.directory: Directory where to store output and results
    #                     (default: .)
    ...
    #                        0 = no weight (KS),
    #                        1 = standard weigth,
    #                        2 = over-weigth (default: 1)
    #   nom.p.val.threshold: Significance threshold for nominal p-vals
    #                        for gene sets (default: -1, no thres)
    #   fwer.p.val.threshold: Significance threshold for FWER p-vals
    #                         for gene sets (default: -1, no thres)
    #   fdr.q.val.threshold: Significance threshold for FDR q-vals for
    #                        gene sets (default: 0.25)
    #   topgs: Besides those passing test, number of top scoring gene sets
    #          used for detailed reports (default: 10)
    #   adjust.FDR.q.val: Adjust the FDR q-vals (default: F)
    #   gs.size.threshold.min: Minimum size (in genes) for database gene sets
    #                          to be considered (default: 25)
    #   gs.size.threshold.max: Maximum size (in genes) for database gene sets
    #                          to be considered (default: 500)
    #   reverse.sign: Reverse direction of gene list
    #                 (pos. enrichment becomes negative, etc.) (default: F)
    #   preproc.type: Preprocessing normalization:
    #                 0=none, 1=col(z-score)., 2=col(rank)
    #                 and row(z-score)., 3=col(rank). (default: 0)
    #   random.seed: Random number generator seed. (default: 123456)
    #   perm.type: Permutation type: 0 = unbalanced, 1 = balanced.
    #              For experts only (default: 0)
    #   fraction: Subsampling fraction. Set to 1.0 (no resampling).
    #             For experts only (default: 1.0)
    #   replace: Resampling mode (replacement or not replacement).
    #            For experts only (default: F)
    #   OLD.GSEA: if TRUE compute the OLD GSEA of Mootha et al 2003
    #   use.fast.enrichment.routine: if true it uses a faster version to
    #                                compute random perm. enrichment
    ...
    #     for the entire sorted dataset, and d.- p-values vs. NES plot.
    #     These plots are in a single JPEG file named
    #     <doc.string>.global.plots.<phenotype>.jpg. When the program is
    #     run interactively these plots appear on a window in the R GUI.
    #   - A variable number of tab-separated gene result text files according
    #     to how many sets pass any of the significance thresholds
    #     ("nom.p.val.threshold," "fwer.p.val.threshold," and
    #     "fdr.q.val.threshold") and how many are specified in the "topgs"
    #     parameter. These files are named:
    #     <doc.string>.<gene set name>.report.txt.
    #   - A variable number of gene set plots (one for each gene set
    #     report file). These plots include a.- Gene set running enrichment
    #     "mountain" plot, b.- gene set null distribution and
    #     c.- heat map for genes in the gene set. These plots are stored in a
    ...
    #               (from the null distribution of the gene set).
    ...
    #   CORE_ENRICHMENT: is this gene is the "core enrichment"
    #                    section of the list? Yes or No variable specifying
    #                    in the gene location is before (positive ES) or after
    #                    (negative ES) the running enrichment peak.
    #
    #   The rows are sorted by the gene location in the gene list.
    #
    # The function call to GSEA returns a  two element list containing
    # the two global result reports as data frames ($report1, $report2).
    #
    #   esults1: Global output report for first phenotype
    ...
    print("Copy input parameters to log file")
    ...
    if (is.data.frame(input.ds)) {
        # write(paste("input.ds=", quote(input.ds), sep=" "),
        # file=filename, append=T)
    } else {
        write(paste("input.ds=", input.ds, sep=" "),
                file=filename, append=T)
    }
    ...
    if (is.data.frame(gene.ann)) {
        # write(paste("gene.ann =", gene.ann, sep=" "),
        # file=filename, append=T)
    } else {
        write(paste("gene.ann =", gene.ann, sep=" "),
                file=filename, append=T)
    ...
    print("Start of GSEA methodology")
    ...
    # preproc.type control the type of pre-processing:
    # threshold, variation filter, normalization
    ...
    print("Sort samples according to phenotype")
    ...
    print("Read input gene set database")
    ...
    for (i in 1:max.Ng) {
        ...
        if ((set.size < gs.size.threshold.min) ||
            (set.size > gs.size.threshold.max)) next
    ...
    print(paste("Read gene and gene set annotations",
                "if gene annotation file was provided"))
    ...
    if (is.data.frame(gene.ann)) {
        ...
        temp <- read.delim(gene.ann, header=T, sep=",",
    ...
    # GSEA methodology
    print("GSEA methodology")

    # Compute observed and random permutation gene rankings
    print("Compute observed and random permutation gene rankings")
    ...
    for (nk in 1:n.tot) {
        ...
        print(paste("Computing ranked list for actual and",
                    "permuted phenotypes.......permutations: ",
                    n.starts[nk], "--", n.ends[nk], sep=" "))

        O <- GSEA.GeneRanking(A, class.labels, gene.labels,
                              call.nperm, permutation.type = perm.type,
                              sigma.correction = "GeneCluster",
    ...
    print("Compute enrichment for random permutations")
    ...
    if (reshuffling.type == "sample.labels") { # reshuffling phenotype labels
        for (i in 1:Ng) {
            print(paste("Computing random permutations'",
                        "enrichment for gene set:", i, gs.names[i], sep=" "))
            gene.set <- gs[i,gs[i,] != "null"]
            gene.set2 <- vector(length=length(gene.set), mode = "numeric")
            gene.set2 <- match(gene.set, gene.labels)
            for (r in 1:nperm) {
            ...
            if (fraction < 1.0) {
                ...
                # if no resampling then compute only one column
                # (and fill the others with the same value)
        ...
        for (i in 1:Ng) {
            ...
            if (fraction < 1.0) {
                # if resampling then compute ES for all
                # observed rankings
                for (r in 1:nperm) {
                    obs.gene.list2 <- obs.order.matrix[,r]
                    if (use.fast.enrichment.routine == F) {
    ...
    print("Compute 3 types of p-values")
    ...
    if (OLD.GSEA == F) {
        ...
        # For OLD GSEA compute the p-val using positive and negative values
    ...
    KS.mean <- function(N) { # KS mean as a function of set size N
        ...
        for (k in -100:100) {
            ...
            S <- S + 4 * (-1)**(k + 1) * (
                0.25 * exp(-2 * k * k * N) -
                sqrt(2 * pi) * erf(sqrt(2 * N) * k)/(16 * k * sqrt(N))
            )
        }
        return(abs(S))
    }
    ...
    print("Save intermedite results")

    if (save.intermediate.results == T) {

        filename <- paste(output.directory, doc.string,
                          ".phi.txt", sep="", collapse="")
        write.table(phi, file = filename, quote=F, col.names= F,
                    row.names=F, sep = "\t")

        filename <- paste(output.directory, doc.string,
                          ".obs.phi.txt", sep="", collapse="")
        write.table(obs.phi, file = filename, quote=F,
                    col.names= F, row.names=F, sep = "\t")

        filename <- paste(output.directory, doc.string,
                          ".phi.norm.txt", sep="", collapse="")
        write.table(phi.norm, file = filename, quote=F,
                    col.names= F, row.names=F, sep = "\t")

        filename <- paste(output.directory, doc.string,
                          ".obs.phi.norm.txt", sep="", collapse="")
        write.table(obs.phi.norm, file = filename, quote=F,
                    col.names= F, row.names=F, sep = "\t")

        filename <- paste(output.directory, doc.string,
                          ".Obs.ES.txt", sep="", collapse="")
        write.table(Obs.ES, file = filename, quote=F,
                    col.names= F, row.names=F, sep = "\t")

        filename <- paste(output.directory, doc.string,
                          ".Obs.ES.norm.txt", sep="", collapse="")
        write.table(Obs.ES.norm, file = filename, quote=F,
                    col.names= F, row.names=F, sep = "\t")
    }
    ...
    if (OLD.GSEA == F) {
        ...
        for (i in 1:Ng) {
            ...
            if (Obs.ES.norm[i] >= 0) {
                p.vals[i, 2] <- signif(sum(max.ES.vals.p >= ES.value)/
                                       length(max.ES.vals.p), digits=5)
            } else {
                p.vals[i, 2] <- signif(sum(max.ES.vals.n <= ES.value)/
                                       length(max.ES.vals.n), digits=5)
            }
        }
    } else {
        # For OLD GSEA compute the FWER using positive and
        # negative values in the same histogram
        max.ES.vals <- NULL
        for (j in 1:nperm) {
        ...
        for (i in 1:Ng) {
            if (Obs.ES.norm[i] >= 0) {
                p.vals[i, 2] <- sum(max.ES.vals >= Obs.ES.norm[i])/
                                length(max.ES.vals)
            } else {
                p.vals[i, 2] <- sum(max.ES.vals <= Obs.ES.norm[i])/
                                length(max.ES.vals)
        ...
        for (i in 1:nperm) {
            ...
            if (ES.value >= 0) {
                ...
                count.col[i] <- ifelse(count.col.norm > 0,
                                       sum(phi.vec >= ES.value)/count.col.norm,
                                       0)
                obs.count.col[i] <- ifelse(
                    obs.count.col.norm > 0,
                    sum(obs.phi.vec >= ES.value)/obs.count.col.norm,
                    0
                )
            } else {
                count.col.norm <- sum(phi.vec < 0)
    ...
    print("Compute global statistic")
        ...
        if (NES[k] >= 0) {
            for (i in 1:nperm) {
                ...
                OBS.pass[i] <- ifelse(OBS.pos > 0,
                                      sum(obs.phi.norm[,i] >= NES[k])/OBS.pos,
                                      0)
            }
        } else {
            for (i in 1:nperm) {
                ...
                OBS.pass[i] <- ifelse(OBS.neg > 0,
                                      sum(obs.phi.norm[,i] <= NES[k])/OBS.neg,
                                      0)
            }
        }
    ...
    report <- data.frame(cbind(gs.names, size.G, all.gs.descs, Obs.ES,
                               Obs.ES.norm, p.vals[,1], FDR.mean.sorted,
                               p.vals[,2], tag.frac, gene.frac,
                               signal.strength, FDR.median.sorted,
                               glob.p.vals.sorted))
    ...
    glob.filename <- paste(output.directory, doc.string,
                           ".global.plots.png",
                           sep="", collapse="")
    png(file=glob.filename, height = devpars$height, width = devpars$width,
        res = devpars$res)

    nf <- layout(matrix(c(1,2,3,4), 2, 2, byrow=T), c(1,1), c(1,1), TRUE)

    # plot S2N correlation profile
    print("plot S2N correlation profile")
    ...
    x <- plot(location, obs.s2n, ylab = "Signal to Noise Ratio (S2N)",
              xlab = "Gene List Location",
              main = "Gene List Correlation (S2N) Profile",
              type = "l", lwd = 2, cex = 0.9, col = 1)
    for (i in seq(1, N, 20)) {
        lines(c(i, i), c(0, obs.s2n[i]), lwd = 3, cex = 0.9,
              col = colors()[12]) # shading of correlation plot
    }
    ...
    lines(c(arg.correl, arg.correl), c(min.corr, 0.7*max.corr),
          lwd = 2, lty = 3, cex = 0.9, col = 1) # zero correlation vertical line

    area.bias <- signif(100*(sum(obs.s2n[1:arg.correl]) +
                             sum(obs.s2n[arg.correl:N]))/
                             sum(abs(obs.s2n[1:N])), digits=3)
    ...
    if (Ng > 1) { # make these plots only if there are multiple gene sets.
        ...
        print("Compute plots of actual (weighted) null and observed")
        ...
        #  lines(c(x1.point[i], x1.point[i]), c(0, y1.point[i]),
        #        lwd = 3, cex = 0.9, col = colors()[555]) # shading
        # }
        #
        # for (i in 1:length(x2.point)) {
        #  lines(c(x2.point[i], x2.point[i]), c(0, y2.point[i]),
        #        lwd = 3, cex = 0.9, col = colors()[29]) # shading
        # }
        ...
        legend(x=0, y=1.5*y.plot.range[2], bty="n", bg = "white",
               legend=leg.txt, lty = lty.vec, lwd = lwd.vec,
               col = c.vec, cex = 0.9)

        B <- A[obs.index,]
        if (N > 300) {
            C <- rbind(B[1:100,], rep(0, Ns), rep(0, Ns),
                       B[(floor(N/2) - 50 + 1):(floor(N/2) + 50),],
                       rep(0, Ns), rep(0, Ns), B[(N - 100 + 1):N,])
        } else {
            # modified by pwwang@pwwang.com, in case # genes <= 300
            C <- B
        }
        rm(B)
        GSEA.HeatMapPlot(V = C, col.labels = class.labels,
                         col.classes = class.phen,
        ...
        plot(NES, FDR.mean, ylim = c(0, 1), xlim = plot.range, col = 1,
             bg = 1, type="p", pch = 22, cex = 0.75, xlab = "NES",
        ...
        lines(c(min(NES), max(NES)),
              c(nom.p.val.threshold, nom.p.val.threshold),
              lwd = 1, lty = 2, col = 2)
        lines(c(min(NES), max(NES)),
              c(fwer.p.val.threshold, fwer.p.val.threshold),
    ...
    #-------------------------------------------------------------------
    # Produce report for each gene set passing the nominal,
    # FWER or FDR test or the top topgs in each side
    print(paste("Produce report for each gene set passing the nominal,",
                "FWER or FDR test or the top topgs in each side"))
    ...
    for (i in 1:Ng) {
        if ((p.vals[i, 1] <= nom.p.val.threshold) ||
            ...
            (is.element(i, c(Obs.ES.index[1:topgs],
            ...
            for (k in set.k) {
                if (Obs.indicator[i, k] == 1) {
                    ...
                    if (Obs.ES[i] >= 0) {
                        core.enrichment[kk] <- ifelse(
                            gene.list.loc[kk] <= Obs.arg.ES[i], "YES", "NO"
                        )
                    } else {
                        core.enrichment[kk] <- ifelse(
                            gene.list.loc[kk] > Obs.arg.ES[i], "YES", "NO"
            ...
            gene.report <- data.frame(cbind(gene.number, gene.names,
                                            gene.symbols, gene.descs,
                                            gene.list.loc, gene.s2n,
            ...
            gs.filename.res <- paste(output.directory,
                               doc.string, ".",
                               gs.names[i], ".plot.",
                               phen.tag, ".", loc, ".res.png",
                               sep="", collapse="")
            png(file=gs.filename.res, height = devpars$height,
                width = devpars$width, res = devpars$res)
            ...
            max.corr <- max(obs.s2n)
            min.corr <- min(obs.s2n)
            Obs.correl.vector.norm <- (obs.s2n - min.corr)/
                                      (max.corr - min.corr)*1.25*
                                      delta + min.plot
            ...
            # zero RES line
            lines(c(1, N), c(0, 0), lwd = 1, lty = 2, cex = 1, col = 1)
            # max enrichment vertical line
            lines(c(Obs.arg.ES[i], Obs.arg.ES[i]), c(min.plot, max.plot),
                  lwd = 1, lty = 3, cex = 1, col = col)
            for (j in 1:N) {
                if (Obs.indicator[i, j] == 1) {
                    # enrichment tags
                    lines(c(j, j),
                          c(min.plot + 1.25*delta, min.plot + 1.75*delta),
            ...
            lines(ind, Obs.correl.vector.norm, type = "l", lwd = 1,
                  cex = 1, col = 1)
            # zero correlation horizontal line
            lines(c(1, N), c(zero.corr.line, zero.corr.line), lwd = 1,
                             lty = 1, cex = 1, col = 1)
            temp <- order(abs(obs.s2n), decreasing=T)
            arg.correl <- temp[N]
            # zero crossing correlation vertical line
            ...
            gs.filename.dist <- paste(output.directory,
                                      doc.string, ".",
                                      gs.names[i], ".plot.",
                                      phen.tag, ".", loc, ".dist.png",
                                      sep="", collapse="")
            png(file=gs.filename.dist, height = devpars$height,
                width = devpars$width, res = devpars$res)

            sub.string <- paste("ES =", signif(Obs.ES[i], digits = 3),
                                " NES =", signif(Obs.ES.norm[i], digits=3),
                                "Nom. p-val=", signif(p.vals[i, 1], digits = 3),
                                "FWER=", signif(p.vals[i, 2], digits = 3),
                                "FDR=", signif(FDR.mean.sorted[i], digits = 3))
            ...
            gs.filename.heat <- paste(output.directory,
                                      doc.string, ".",
                                      gs.names[i], ".plot.",
                                      phen.tag, ".", loc, ".heatmap.png",
                                      sep="", collapse="")
            png(file=gs.filename.heat, height = devpars$height,
                width = devpars$width, res = devpars$res)
            ...
            GSEA.HeatMapPlot(V = pinko, row.names = pinko.gene.names,
                             col.labels = class.labels,
                             col.classes = class.phen,
                             col.names = sample.names,
                             class.tags = class.tags,
                             main = paste(" Heatmap for genes in Gene Set:",
                                          gs.names[i]),
                             xlab = " ", ylab = " ")
...
GSEA.ConsPlot <- function(V, col.names, main = " ",
                          sub = " ", xlab=" ", ylab=" ") {
...
GSEA.HeatMapPlot2 <- function(V, row.names = "NA", col.names = "NA",
                              main = " ", sub = " ", xlab=" ", ylab=" ",
                              color.map = "default") {
    #
    ...
    if (color.map == "default") {
        color.map <- rev(rainbow(100, s = 1.0, v = 0.75, start = 0.0,
                                 end = 0.75, gamma = 1.5))
    }
...
#            size.col.char <- ifelse(n.cols < 2.5, 1, sqrt(2.5/n.cols))
        ...
        axis(2, at=1:n.rows, labels=row.names, adj= 0.5, tick=FALSE,
             las = 1, cex.axis=size.row.char, font.axis=1, line=-1)
    }
...
GSEA.Analyze.Sets <- function(directory,
                              topgs = "",
                              height = 12,
                              width = 17,
                              res = 100) {
    ...
    # Analysis for phen1
    print("Analysis for phen1")
    if (is.na(set.table.phen1[1, 1])) {
        print("  Nothing found for phen1, skip")
    } else {
        leading.lists <- NULL
        for (i in 1:max.sets.phen1) {
            ...
            gene.set <- read.table(file=inputfile, sep="\t", header=T,
                                comment.char="", as.is=T)
            leading.set <- as.vector(
                gene.set[gene.set[,"CORE_ENRICHMENT"]=="YES", "SYMBOL"]
            )
        ...
        for (i in 1:max.sets.phen1) {
            # notice that the sign is 0 (no tag) or 1 (tag)
            M[i,] <- sign(match(all.leading.genes,
                                as.vector(leading.lists[[i]]),
                                nomatch=0))
        }
        ...
        for (i in 1:max.sets.phen1) {
            for (j in 1:max.sets.phen1) {
                Inter[i, j] <- length(
                    intersect(leading.lists[[i]], leading.lists[[j]])
                )/length(union(leading.lists[[i]], leading.lists[[j]]))
            }
        }
        ...
        output.line <- paste(row.header, row.header, output.line,
                            sep="\t", collapse="")
        output <- paste(directory, doc.string, ".all.leading.genes.",
                        phen1, ".gmt", sep="")
        write(noquote(output.line), file = output,
              ncolumns = length(output.line))

        filename <- paste(directory, doc.string, ".leading.assignment.",
                          phen1, ".png", sep="", collapse="")
        png(file=filename, height = height, width = width, res = res)

        cmap <-  c("#AAAAFF", "#111166")
        GSEA.HeatMapPlot2(V = data.matrix(D.phen1),
                          row.names = row.names(D.phen1),
                          col.names = names(D.phen1),
                          main = "Leading Subsets Assignment",
                          sub = paste(doc.string, " - ", phen1, sep=""),
                          xlab=" ", ylab=" ", color.map = cmap)
    ...
    # Analysis for phen2
    print("Analysis for phen2")

    if (is.na(set.table.phen2[1, 1])) {
        print("  Nothing found for phen2, skip")
    } else {
        leading.lists <- NULL
        ...
        for (i in 1:max.sets.phen2) {
            # notice that the sign is 0 (no tag) or 1 (tag)
            M[i,] <- sign(match(all.leading.genes,
                                as.vector(leading.lists[[i]]), nomatch=0))
        }
        ...
        print("Save primary subsets as a single gene set in a .gmt file")
        ...
        cmap <-  c("#AAAAFF", "#111166")
        GSEA.HeatMapPlot2(V = data.matrix(D.phen2),
                          row.names = row.names(D.phen2),
                          col.names = names(D.phen2),
                          main = "Leading Subsets Assignment",
                          sub = paste(doc.string, " - ", phen2, sep=""),
                          xlab=" ", ylab=" ", color.map = cmap)
    ...
    print("Resort columns and rows for phen1")
    ...
    if (is.na(set.table.phen1[1, 1])) {
        ...
        #   GSEA.HeatMapPlot2(V = A, row.names = A.row.names,
        #                     col.names = A.names,
        #                     sub = "  ",
        #                     main = paste("Primary Sets Assignment - ",
        #                     doc.string, " - ", phen1, sep=""),
        #                     xlab=" ", ylab=" ")
        ...
        #   GSEA.HeatMapPlot2(V = A, row.names = A.row.names,
        #                     col.names = A.names,
        #                     main = "Leading Subsets Assignment (clustered)",
        #                     sub = paste(doc.string, " - ", phen1, sep=""),
        #                     xlab=" ", ylab=" ", color.map = cmap)

        GSEA.HeatMapPlot2(V = t(A), row.names = A.names,
                          col.names = A.row.names,
                          main = "Leading Subsets Assignment (clustered)",
                          sub = paste(doc.string, " - ", phen1, sep=""),
                          xlab=" ", ylab=" ", color.map = cmap)

        text.filename <- paste(directory, doc.string,
                            ".leading.assignment.clustered.", phen1,
                            ".txt", sep="", collapse="")
        ...
        write.table(t(A), file=text.filename, append = T, quote=F,
    ...
    print("Resort columns and rows for phen2")
    ...
    if (is.na(set.table.phen2[1, 1])) {
        ...
        #  windows(width=14, height=10)
        #  GESA.HeatMapPlot2(V = A, row.names = A.row.names,
        #                    col.names = A.names,
        #                    sub = "  ",
        #                    main = paste("Primary Sets Assignment - ",
        #                    doc.string, " - ", phen2, sep=""),
        #                    xlab=" ", ylab=" ")
...
#################### GSEA funcitons loaded ######################

# GSEA 1.0 -- Gene Set Enrichment Analysis / Broad Institute
#
# R script to run GSEA Analysis of the Gender vs C1 example
# (cut and paste into R console)
...
if (nthread > 1) {
    ...
    gmtfiles = c()
    for (i in 1:nthread) {
        outdir   = paste(file.path("{{o.outdir}}", paste("Thread-", i, sep="")),
                         '/', sep="")
        dir.create(outdir, showWarnings=F)
        gmtfile  = file.path(outdir, paste('Thread-', i, '.gmt', sep=""))
        outdirs  = c(outdirs, outdir)
        gmtfiles = c(gmtfiles, gmtfile)
        gmtconn  = file(gmtfile)
        writeLines(gmtlines[((i-1)*gmtsize + 1):min(
            i*gmtsize,length(gmtlines)
        )], gmtconn)
        close(gmtconn)
    }
    ...
    foreach(i=1:nthread, .verbose=T) %dopar% capture.output({
        ...
        GSEA(
            ...
            output.directory = outdirs[i],
            #  Program parameters :-----------------------------------------
            # Documentation string used as a prefix to name result files
            # (default: "GSEA.analysis")
            doc.string = "GSEA",
            # Type of permutation reshuffling: "sample.labels" or "gene.labels"
            # (default: "sample.labels"
            reshuffling.type = "sample.labels",
            # Number of random permutations (default: 1000)
            nperm = {{args.nperm}},
            # Enrichment correlation-based weighting:
            # 0=no weight (KS), 1= weigthed, 2 = over-weigthed (default: 1)
            weighted.score.type = {{args.weightexp}},
            # Significance threshold for nominal p-vals for gene sets
            # (default: -1, no thres)
            nom.p.val.threshold = -1,
            # Significance threshold for FWER p-vals for gene sets
            # (default: -1, no thres)
            fwer.p.val.threshold = -1,
            # Significance threshold for FDR q-vals for gene sets
            # (default: 0.25)
            fdr.q.val.threshold = 0.25,
            # Besides those passing test, number of top scoring gene sets
            # used for detailed reports (default: 10)
            topgs = 20,
            # Adjust the FDR q-vals (default: F)
            adjust.FDR.q.val = F,
            # Minimum size (in genes) for database gene sets to be considered
            # (default: 25)
            gs.size.threshold.min = 15,
            # Maximum size (in genes) for database gene sets to be considered
            # (default: 500)
            gs.size.threshold.max = 500,
            # Reverse direction of gene list
            # (pos. enrichment becomes negative, etc.) (default: F)
            reverse.sign = F,
            # Preproc.normalization:
            # 0=none, 1=col(z-score).,
            # 2=col(rank) and row(z-score)., 3=col(rank). (def: 0)
            preproc.type = 0,
            # Random number generator seed. (default: 123456)
            random.seed = 123456,
            # For experts only. Permutation type:
            # 0 = unbalanced, 1 = balanced (default: 0)
            perm.type = 0,
            # For experts only. Subsampling fraction.
            # Set to 1.0 (no resampling) (default: 1.0)
            fraction = 1.0,
            # For experts only, Resampling mode (replacement or not replacement)
            # (default: F)
            replace = F,
            # For experts only, save intermediate results
            # (e.g. matrix of random perm. scores) (default: F)
            save.intermediate.results = F,
            # Use original (old) version of GSEA (default: F)
            OLD.GSEA = F,
            # Use faster routine to compute enrichment for random permutations
            # (default: T)
            use.fast.enrichment.routine = T,
            # devpars for plots
            devpars = devpars
        ...
        #---------------------------------------------------
        # Overlap and leading gene subset assignment analysis
        # of the GSEA results

        GSEA.Analyze.Sets(
            # Directory where to store output and results (default: "")
            directory = outdirs[i],
            # number of top scoring gene sets used for analysis
            topgs = 20,
            height = devpars$height,
            width = devpars$width,
            res = devpars$res
        )
    }, file = file.path(outdirs[i], "log.txt"))
    stopCluster(cl)
} else {
    GSEA(
        # Input/Output Files :-------------------------------------------
        ...
        gs.size.threshold.max = 500,
        # Reverse direction of gene list
        # (pos. enrichment becomes negative, etc.)
        # (default: F)
        reverse.sign = F,
        # Preproc.normalization:
        # 0=none, 1=col(z-score).,
        ...
        # Random number generator seed. (default: 123456)
        random.seed = 111,
        # For experts only. Permutation type: 0 = unbalanced, 1 = balanced
        # (default: 0)
        perm.type = 0,
        # For experts only. Subsampling fraction. Set to 1.0 (no resampling)
        # (default: 1.0)
        fraction = 1.0,
        # For experts only, Resampling mode (replacement or not replacement)
        # (default: F)
...
# GSEA not exit on Errors
stderr <- {{job.errfile | quote}}
errors <- readChar(stderr, file.info(stderr)$size)
if (grepl("Error", errors, fixed = TRUE)) {
