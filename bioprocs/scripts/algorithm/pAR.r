...
inD  = {% if in.D  | lambda x: x.endswith('.gz') %}gzfile({{in.D  | quote}}){% else %}{{in.D  | quote}}{% endif %}
inPt = {% if in.Pt | lambda x: x.endswith('.gz') %}gzfile({{in.Pt | quote}}){% else %}{{in.Pt | quote}}{% endif %}
inY  = {% if in.Y  | lambda x: x.endswith('.gz') %}gzfile({{in.Y  | quote}}){% else %}{{in.Y  | quote}}{% endif %}
...
write.table(round(W, 4), {{out.W | quote}}, quote = F, sep = "\t")

{% if args.predY %}
predY = D %*% W %*% testPt
predyfile = file.path({{out.outdir | quote}}, 'predY.txt')
...
predycorfile = file.path({{out.outdir | quote}}, 'predYcor.txt')
write.table(spcors, predycorfile, sep="\t", quote=F, row.names = F)

predycmsfile = file.path({{out.outdir | quote}}, 'predYcorMeanSd.txt')
...
{% if args.WPt %}
outfile = file.path({{out.outdir | quote}}, 'WPt.txt')
write.table(W %*% as.matrix(Pt), outfile, sep="\t", quote=F)
{% endif %}

{% if args.WtDtY %}
outfile = file.path({{out.outdir | quote}}, 'WtDtY.txt')
