...
we need to split the data into batches and remove the intermediate files for each batch.
"""
import math
...
params.ppldir.desc = ('The directory for PyPPL pipeline where the intermediate files are saved.'
                      'This directory will be clearned up if `dropim` is `True`')
...
def split_batches(saminfo, batchsize):
    ...
    if saminfo.is_paired():
        ...
        for patient in saminfo.all_patients():
            ret.append(tuple(saminfo.get_samples(by='Patient', value=patient, return_all=True)))
    else:
...
def get_current_batch(batches, saminfo, cachefile, metadir, batchsize):
    ...
    if cachefile.is_file():
        ...
        if not Path(cache.saminfo).samefile(saminfo) or cache.batchsize != batchsize:
            cache = False
        else:
            ...
            if not batch_saminfo.is_file():
                ...
                batch_samples = (sum(([sample[0].Sample, sample[1].Sample]
                                 ...
                                 else [sample.Sample for sample in curr_batch])
...
def save_batch_saminfo(samrows, batch, saminfo, metadir):
    ...
    batch_saminfo = metadir.joinpath('%s-batch%s.saminfo' % (saminfo.stem, batch))
    ...
    for samples in samrows:
        if isinstance(samples, tuple):
            writer.write([samples[0].Sample, samples[0].Patient, samples[0].Group])
            writer.write([samples[1].Sample, samples[1].Patient, samples[1].Group])
        else:
...
def main():
    ...
    curr = get_current_batch(batches,
                             opts.saminfo,
                             opts.cachefile,
                             opts.metadir,
                             opts.batchsize)
    ...
    for i in range(curr, len(batches)):
        ...
        logfile = opts.metadir.joinpath('%s-batch%s.log' % (opts.saminfo.stem, i))
        logfile.write_text('')

        batch_saminfo = save_batch_saminfo(batches[i], i, opts.saminfo, opts.metadir)
