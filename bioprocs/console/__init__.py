    ...
    if opts['get']:
        ...
        for key in sorted(params._dict()):
            ...
            for query in queries:
                param = params[key]
                if (query not in param.str().lower()
        ...
        print('\n'.join(
            ...
            for line in HELP_ASSEMBLER.assemble(pminfo)),
              end='')
...
def complete(opts):
    ...
    comp = Completions(desc='Bioprocs utilities.') # pylint: disable=not-callable
    ...
    if command == 'params':
        ...
        if '.' in opts._:  # process
            ...
            try:
                ...
                raise KeyError('Module %r does not have process: %s' %
                               (module, prc)) from None
        else:  # assume module
