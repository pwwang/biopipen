...
.get.aes.string <- function(data, ...) {
    # assuming: `colnames(data) = c('A', 'B', 'C')`
    ...
    for (i in 1:length(anames)) {
        aname <- anames[i]
        colidx <- cols[[aname]]
        ret[[aname]] <- ifelse(is.numeric(colidx),
                               cnames[colidx], cols[[aname]])
    }
    return(ret)
}
...
# #namesbefore = c('data')
update.aes <- function(alist, newaes, namesbefore = c()) { # nolint
    alnames <- names(alist)
...
plot.no <- function(data, plotfile = NULL, params = list(), ggs = list(),
                    devpars = DEVPARS) {
...
plot.xy <- function(data, plotfile = NULL, x = 1, y = 2, ggs = list(),
                    devpars = DEVPARS) {
    cnames <- colnames(data)
...
plot.stack <- function(data, plotfile, x = "ind", y = "values",
...
plot.roc <- function(data,
                     plotfile = NULL,
                     stacked = FALSE,
                     params = list(returnTable = FALSE, showAUC = TRUE,
                                   bestCut = FALSE),
                     ggs = list(),
                     devpars = DEVPARS) {
    # plot the ROC curve
    ...
    if (stacked) {
        if (ncol(data) != 3) {
              stop(
                  "Expect 3 columns (D, M, Group) in stacked data to plot ROC."
              )
          }
        ...
        data <- cbind(rep(data[, 1], ncols - 1),
                      stack(data[, 2:ncols, drop = FALSE]))
        colnames(data) <- c("D", "M", "Group")
    }
    ...
    if (bestCut) {
        ...
        ret$plot <- ret$plot + geom_point(
            ...
            aes(x = specificity, xend = specificity, y = sensitivity,
                yend = -Inf, color = Group),
            data = bestdata,
            linetype = "dashed",
            inherit.aes = FALSE
        ) + geom_segment(
            aes(x = Inf, xend = specificity, y = sensitivity,
                yend = sensitivity, color = Group),
            data = bestdata,
            linetype = "dashed",
            inherit.aes = FALSE
        ) + geom_text_repel(
            aes(
                ...
                label = paste0("best (", round(specificity, 3),
                               ", ", round(sensitivity, 3), ")"),
                color = Group
            ),
            data = bestdata
    ...
    if (showAUC) {
        ...
        if (nrow(aucs) == 1) {
            ret$plot <- ret$plot + annotate("text",
                x = .1, y = .05,
            ...
            ret$plot <- ret$plot + scale_color_discrete(
                name = "",
                breaks = groups,
...
plot.scatter <- function(data, plotfile = NULL, x = 1, y = 2, params = list(),
                         ggs = list(), devpars = DEVPARS) {
...
plot.boxplot <- function(data, plotfile = NULL, x = 2, y = 1, stacked = TRUE,
                         params = list(), ggs = list(), devpars = DEVPARS) {
    if (stacked) {
        cnames <- colnames(data)
        cnames <- make.names(cnames)
        colnames(data) <- cnames
        ...
        ggs <- c(
            ...
            list(theme = list(axis.title.x = element_blank(),
...
plot.violin <- function(data, plotfile = NULL, x = 2, y = 1, stacked = TRUE,
                        params = list(), ggs = list(), devpars = DEVPARS) {
    if (!stacked) {
...
plot.heatmap <- function(data, plotfile, params = list(dendro = T),
                         ggs = list(), devpars = DEVPARS) {
    require("ggdendro")
    require("gtable")
    require("grid")
    ...
    if (dendro == TRUE || dendro == "both" || dendro == "col") {
        ...
        plotdtop <- NULL
        colidx <- seq_len(cnames)
    }

    mm <- stack(as.data.frame(data[rowidx, colidx]))
    mm$rnames <- rnames[rowidx]
    plothm <- ggplot(mm, aes(ind, rnames)) +
        geom_tile(aes(fill = values)) +
        xlim(unique(as.vector(mm$ind))) +
        scale_y_discrete(position = "right", limits = rnames)
...
plot.histo <- function(data, plotfile = NULL, x = 1, params = list(),
                       ggs = list(), devpars = DEVPARS) {
...
plot.density <- function(
    ...
    aes.string <- .get.aes.string(data, x = x, y = y)
    params <- c(params, list(mapping = aes_string(fill = aes.string$y),
                             alpha = .3))
...
plot.freqpoly <- function(data, plotfile, x = 1, params = list(), ggs = list(),
                          devpars = DEVPARS) {
    ggs <- c(list(geom_freqpoly = params), ggs)
    plot.x(data, plotfile, x, ggs, devpars)
}

plot.maplot <- function(data, plotfile, threshold, ggs = list(),
                        devpars = DEVPARS) {
    ...
    plot.scatter(data, plotfile, x = "A", y = "M", params = params, ggs = ggs,
                 devpars = devpars)
}
...
plot.qq <- function(data, plotfile = NULL, x = NULL, y = 1, params = list(),
    ...
    plot.scatter(data, plotfile, x = x, y = y, params = params, ggs = ggs,
                 devpars = devpars)
}
...
# item4	1		0		0
# item5	1		0		1
plot.venn <- function(data, plotfile = NULL, params = list(),
                      devpars = DEVPARS) {
    library(VennDiagram)
    rnames <- rownames(data)
    if (is.null(rnames)) {
...
plot.upset <- function(data, plotfile = NULL, params = list(),
                       devpars = DEVPARS) {
    library(UpSetR)
...
#           both up-/down genes, or
# 		- a list of cutoffs for logfc and p/q value
# 			- list(logfc = 2, p = .05), or if you want have different
#               logfc cutoff for up-/down genes:
...
plot.volplot <- function(data,
    ...
    if (is.null(params$hilight)) {
        ...
        labeldata <- data[(data$logFC <= logfc[1] | data$logFC >= logfc[2]) &
                           data[, 2] > p, ,
                          drop = FALSE]
        labeldata$Label <- rownames(labeldata)
    }
    ...
    vline <- list(
        xintercept = c(-params$logfccut, params$logfccut),
        linetype = "dashed", color = c("red3", "blue3")
    )
    vtext1 <- list(
        x = -params$logfccut, y = 0,
        label = paste(-params$logfccut, "fold"),
        vjust = 1,
        hjust = 1.1,
        color = "red3"
    )
    vtext2 <- list(
        x = +params$logfccut,
        y = 0,
        label = paste0("+", params$logfccut, " ", "fold"),
        vjust = 1,
        hjust = -0.1,
        color = "blue3"
    )
    ...
    if (!is.null(labeldata)) {
        labels <- list(
            aes_string(
                x = "logFC",
                y = ifelse(is.pval, "log10.P.", "log10.FDR."),
                label = "Label",
                color = "Group"
            ),
            alpha = 1,
...
plot.man <- function(data, plotfile = NULL, hilights = list(), hilabel = TRUE,
    ...
    # calculate the x axis for each snp,
    # position on each chromsome should be accumulated.
    ...
    if (is.null(gsize)) {
        ...
        chrlen <- data[, .(minPos = min(Pos),
                           maxPos = max(Pos)),
                           by = Chr][, .(
            Chr,
            minPos,
            maxPos,
            cumPos = shift(cumsum(as.numeric(maxPos)), 1, fill = 0)
        )]
        # add it back to data
    } else {
        chrs <- unique(unlist(data[, "Chr"]))
        gsize <- gsize[chrs, , drop = FALSE]
        gsize <- as.data.table(cbind(Chr = rownames(gsize),
                                     minPos = 0,
                                     maxPos = gsize[, 1]))
        gsize <- gsize[Chr %in% chrs]
        chrlen <- gsize[, .(Chr, minPos, maxPos,
                            cumPos = shift(cumsum(as.numeric(maxPos)), 1,
                                           fill = 0))]
    }
    ...
    for (hilight in hilights) {
        if ("snps" %in% names(hilight)) {
            ...
            hdata_tmp <- data[Chr == hilight$chr &
                              Pos >= as.numeric(hilight$start) &
                              Pos <= as.numeric(hilight$end)]
            hdata <- ifelse(is.null(hdata), hdata_tmp, rbind(hdata, hdata_tmp))
            rm(hdata_tmp)
        } else {
    ...
    if (!is.null(hdata)) {
        ...
        ggs_hilabel <- list()
        if (hilabel) {
            ggs_hilabel <- list(geom_label_repel = list(
                aes_string(x = "X", y = "Y", label = "Snp"),
                color = "white",
                fill = hicolor,
                data = hdata,
                inherit.aes = FALSE,
        ...
        ggs_hilight <- c(
            ggs_hilight,
            ggs_hilabel,
            list(geom_point = list(aes_string(x = "X", y = "Y"),
                                   color = hicolor,
                                   data = hdata,
                                   inherit.aes = FALSE))
        )
    ...
    ggs <- c(list(
        ...
        scale_color_manual = list(values = rep(c("grey", "skyblue"),
                                               nrow(rdata))),
        scale_y_continuous = list(expand = c(0, 0)),
        scale_x_continuous = list(expand = expand_scale(mult = c(0.01, 0.01)),
                                  label = rdata$Region,
                                  breaks = rdata$centerPos),
        theme_bw = list(),
        theme = list(
            panel.grid.minor = element_blank(),
...
.surv.cut <- function(data, vars = NULL, by = "maxstat",
                      labels = c("low", "high"), keep.raw = FALSE) {
    ...
    for (v in vars) {
        ...
        if (by == "asis" || !is.numeric(vdata) ||
            length(levels(factor(vdata))) <= 5) {
            data[, v] <- as.factor(as.character(vdata))
        } else if (by == "maxstat") {
            data.cut <- surv_cutpoint(data, time = cnames[1], event = cnames[2],
                                      variables = v)
            ...
            tmp <- sapply(seq_len(vdata), function(i) {
                if (vdata[i] < vcut) {
                    labels[1]
...
plot.survival.km <- function(
    data,
    plotfile = NULL,
    params = list(risk.table = TRUE,
                  pval = TRUE,
                  cut = "maxstat",
    ...
    #   params:  The params used for ggsurvplot. If provided, global
    #       plot$params will not be override
    # @returns:
    ...
    survcut <- .surv.cut(data, by = cutmethod, labels = cutlabels,
                         keep.raw = TRUE)
    data <- survcut$data

    ret$cutplot <- ifelse(length(survcut$plot) > 0,
                          survcut$plot[[variable]], NULL)
    ret$data <- data
    ...
    if (length(levels(as.factor(data[, 3]))) < 2) {
        ...
        ret$table <- data.frame(
            ...
            test = "LogRank",
            groups = paste(
                apply(counts, 1, function(row) paste(row, collapse = ":")),
                collapse = ", "
            ),
            df = 1,
...
plot.survival.cox <- function(
    data,
    plotfile = NULL,
    params = list(risk.table = TRUE,
                  pval = TRUE,
                  # mean, median, q25, q75, asis, or a value as cutoff
                  # asis uses the levels in 3rd column,
                  # requring categorical values
    ...
    # compose formula
    fmula <- sprintf("Surv(%s, %s) ~ .", bQuote(cnames[1]),
                     bQuote(cnames[2]))
    # do cox regression
    ...
    ret$table <- cbind(
        ret$table, sumfit$conf.int[, -2, drop = FALSE],
        sumfit$coefficients[, 4:5, drop = FALSE]
    )
    ...
    params$cut <- NULL
    params$cutlabels <- NULL
    survcuts <- .surv.cut(data, by = cutmethod, labels = cutlabels,
                          vars = variable, keep.raw = TRUE)
    ret$cutplot <- ifelse(length(survcuts$plot) > 0,
                          survcuts$plot[[variable]], NULL)
    ...
    ret$plot <- do.call(
        ...
        c(list(fit2,
               data = newdata,
               legend.labs = paste(variable, "=", groups)),
          params)
    )

    # need to hack a risk table.
    # nolint See https://github.com/kassambara/survminer/issues/231#issuecomment-309453646
    if (risktable) {
        fmula <- sprintf(
            "Surv(%s, %s) ~ %s",
...
plot.mds <- function(data, plotfile, params = list(has.group = TRUE, ndim = 2),
    ...
    plotdata <- as.data.frame(cmdscale(dist(scale(data)),
                                       k = list.get(params, "ndim", 2)))
    if (!is.null(groupdata)) {
        ...
        for (grup in levels(as.factor(plotdata$Group))) {
            if (is.null(hulldata)) {
                ...
                hulldata <- rbind(
                    hulldata,
                    plotdata[plotdata$Group == grup, , drop = FALSE][
                        chull(plotdata[plotdata$Group == grup, 1:2]),
                    ]
                )
            }
        ...
        ggs <- c(list(
            ...
            geom_polygon = list(aes_string(fill = "Group", group = "Group"),
                                alpha = .3,
                                data = hulldata),
            geom_text_repel = list(label = rownames(plotdata),
                                   min.segment.length = unit(0, "lines")),
            theme_bw = list(),
    ...
    plot.xy(plotdata, plotfile, x = "V1", y = "V2", ggs = ggs,
            devpars = devpars)
}
