...
pBam2Cnv.infile            = 'origin' # the index file should be with the original file
...
pBam2Cnv.args.genome       = params.genome.value # only for cnvnator
pBam2Cnv.args.params       = Box(
	access     = Box(s = 5000),
	target     = Box({'split': True, 'short-names': True}),
	coverage   = Box(),
	reference  = Box({'no-edge': True}),
	fix        = Box({'no-edge': True}),
	segment    = Box(),
	call       = Box(),
	plot       = Box(),
	breaks     = Box(),
	...
	export     = Box(),
	scatter    = Box(), # multiple scatter plots [Box(), Box(), ...]
	heatmap    = Box(), # multiple scatter plots [Box(), Box(), ...]
	diagram    = Box()
)
pBam2Cnv.args.report  = True # cnvkit only
pBam2Cnv.args.plot    = True # cnvkit only
pBam2Cnv.args.binsize = 100 # only for cnvnator
pBam2Cnv.args.type    = 1  # wandy 1:hg19 solid cell/blood, 2:hg19 cell free/plamsa, 3:hg38 solid cell/blood, 4:hg38 cell free/plamsa
