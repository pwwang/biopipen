...
if [ $# -eq 0 ]; then
    ...
    echo "                     biopipen/pipseeker-pipeline:<tag> PipseekerPipeline \\"
    echo "                     @config.toml"
    echo ""
    ...
    echo -e "                     --rm -w /workdir -v .:/workdir \\"
    echo "                     biopipen/pipseeker-pipeline:<tag> board \\"
    echo "                     biopipen.ns.pipseeker_pipeline:PipseekerPipeline -a /biopipen/docker/pipseeker_pipeline/board.toml"
    echo ""
    echo -e "${Color_Off}"
    exit 1
elif [[ "$1" == "board" ]]; then
    ...
    pipen $@
elif [[ "$1" == "PipseekerPipeline" ]]; then
    # shellcheck disable=SC2068
    pipen run pipseeker_pipeline $@
else
