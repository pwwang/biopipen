...
tnsams <- sample(samples, as.integer(if (tfrac < 1) nsams * tfrac else tfrac))
...
trainPt <- scale(t(trainPt), scale = FALSE)

testY <- testY - trainYrmean
testY <- L2(testY)
testP <- sweep(t(testPt), 2, trainPtrmean)
...
if (svdP > 0) {
    ...
    X <- kronecker(trainPt, t(trainY) %*% D)
}
...
X[is.na(X)] <- 0
cvfit <- cv.glmnet(X, vecYtY, alpha = alpha, nfolds = nfold, lower = -1,
                   upper = 1, parallel = nthread > 1)
if (method == "glmnet") {
...
if (predY) {
    ...
    for (tts in ttsams) {
        ...
        # // spcors = rbind(spcors, sample = tts,
        # // cor = spcor$rho, p = spcor$p.value)
    ...
    cormeansd <- matrix(c(mean(as.numeric(spcors[, "cor"])),
                          sd(as.numeric(spcors[, "cor"]))), ncol = 1)
...
if (WtDtY) {
    ...
    write.table(t(W) %*% t(as.matrix(D)) %*% as.matrix(Y), outfile,
                sep = "\t", quote = F)
}
